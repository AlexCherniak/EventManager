...
function fbgetPhoto()
{
	FB.api('/me/picture?type=normal', function(response) {
		...
		var str="<img src='"+response.data.url+"' width='80' height='80' />";
...
function getfbgetSession() {
	FB.api('/me', function(response) {

		var str1=response.id;
		var str="<form action='makeSession' method='post'> <input type='hidden' name='hosterIDd' value='"+str1+"'> <input type='submit' value='click here to create a new event.'></form>";
		document.getElementById("mySession").innerHTML+=str;});

}

function getfbgetSession2() {
	FB.api('/me', function(response) {

		var str1=response.id;
		var str="<form action='makeSession' method='post'> <input type='hidden' name='hosterIDd' value='"+str1+"'> <input type='submit' value='click here to refesh your events list'></form>";
		document.getElementById("mySession2").innerHTML+=str;});
}
...
function myIDforSession() {
	FB.api('/me', function(response) {

		var str1=response.id;
		var str="<%list = DataBaseManager.getInstance().getEvent('"+str1+"');%>";
		document.getElementById("mySession").innerHTML+=str;});
}
